...
export const PathwaysNiivueCanvas = () => (
  ...
  // Handlers for showing MRI, crosshair, and per-tract visibility
  ...
  const handleTractChange = (index, event) => {
    ...
    mesh.indexCount = visible ? originalIndexCounts.current[index] : 0;
    niivue_render.current.updateGLVolume();
    setTractVisibility(prev => prev.map((v, i) => i === index ? visible : v));
  };

  const handleShowAllTracts = (event) => {
    const visible = event.target.checked;
    niivue_render.current.meshes.forEach((mesh, i) => {
      mesh.indexCount = visible ? originalIndexCounts.current[i] : 0;
    });
    niivue_render.current.updateGLVolume();
    setTractVisibility(trackList.map(() => visible));
  };
